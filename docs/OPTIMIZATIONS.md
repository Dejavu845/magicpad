...
| ID | Linux | What |
|---|---|---|
| MP-12 | YES | `HTTP_ROUTE_COUNT = 5`. `/hello` and `/classify` are not routes. Tests in `scripts/test_cycle38_http_count.py`. |

## Cycle 37 — documented HTTP path helper

| ID | Linux | What |
|---|---|---|
| MP-12 | YES | `is_documented_http_path` uses `HTTP_ROUTES`. `/classify` and `/pair` are false. Tests in `scripts/test_cycle37_http_path.py`. |

## Cycle 36 — HTTP route allowlist excludes classify

| ID | Linux | What |
|---|---|---|
| MP-12 | YES | `HTTP_ROUTES` is `/` `/health` `/stt` `/drop` `/cert`. PROTOCOL HTTP table matches. Tests in `scripts/test_cycle36_http_routes.py`. |

## Cycle 35 — print-only QR never emits classify=

| ID | Linux | What |
|---|---|---|
| MP-12 | YES | `generate_qr` source scan rejects `classify=`. `--print-only` stdout has no `classify=` / `pair=`. Tests in `scripts/test_cycle35_print_only.py`. |
...
| ID | Linux | What |
|---|---|---|
| MP-12 | YES | `Classify.httpPath = nil` + `CLASSIFY_HTTP_PATH = None`. PROTOCOL HTTP table has `/stt` `/drop` `/health` and no `/classify`. Tests in `scripts/test_cycle33_classify_http.py`. |

## Cycle 32 — classify never a /health key

| ID | Linux | What |
|---|---|---|
| MP-12 | YES | `Classify.isHealthKey = false` + `CLASSIFY_IS_HEALTH_KEY`. `forbiddenHealthKeys` includes `classify`. PROTOCOL health keys line omits it. Tests in `scripts/test_cycle32_classify_health.py`. |

## Cycle 31 — shared LAN fixture table

| ID | Linux | What |
|---|---|---|
| MP-18 | YES | `scripts/lan_fixtures.py` loads `scripts/fixtures/lan-vectors.json`. Cycle 28 iterates the same rows. Tests in `scripts/test_cycle31_lan_fixtures.py`. |
...
| ID | Linux | What |
|---|---|---|
| MP-18 | YES | `BinaryFrame.parse` + `parse_binary_frame`. &lt;7 bytes → nil / None (no inject). 13-byte pointer + 18-byte gesture. Local `EventInjector` / latency echo use Core. Tests in `scripts/test_cycle26_binary.py`. |

## Cycle 25 — STT onDevice in Core

| ID | Pri | Linux | What landed |
|---|---|---|---|
| MP-12 | P1 | YES | `STTOnDevice.parse` + `parse_stt_on_device`. JSON bool only; missing / non-bool → true. Local `handleSTTControl` uses Core. Tests in `scripts/test_cycle25_stt_ondevice.py`. |

## Cycle 24 — STT lang allowlist in Core

| ID | Pri | Linux | What landed |
|---|---|---|---|
| MP-12 | P1 | YES | `STTLang.parse` + `parse_stt_lang`. Only `zh-CN` / `en-US` / `ja-JP`. Else `zh-CN`. Local `handleSTTControl` uses Core. Tests in `scripts/test_cycle24_stt_lang.py`. |
...
| ID | Pri | Linux | What landed |
|---|---|---|---|
| MP-12 | P1 | YES | PROTOCOL `stt` inbound/status/final field list (from local `handleSTTControl`). |
| docs | P1 | YES | ENGINEERING no longer says POST /drop /stt have no Origin check. Cycle 7 local wire; remote stub unrestored. |

## Cycle 18 — MP-12 hello / ping fields

| ID | Pri | Linux | What landed |
|---|---|---|---|
| MP-12 | P1 | YES | PROTOCOL documents `hello` / `hello_ack` / `ping` / `pong` fields. No pairing token. |
...
| MP-12 | YES | Cycle 19: `stt` JSON fields. Cycle 20 pairing hatch. Cycle 21–22: QR never embeds token. Cycle 23–25: STT action / lang / onDevice in Core. Cycle 30: classify never injects. Cycle 32: classify never a `/health` key. Cycle 33: no HTTP `/classify`. Cycle 34: QR never carries `classify=`. Cycle 35: `--print-only` omits `classify=`. Cycle 36: `HTTP_ROUTES` allowlist. Cycle 37: `is_documented_http_path`. Cycle 38: closed route count. |
...
| ID | Next step |
|---|---|
| MP-18 | Cycle 23: `STTAction`. Cycle 26: `BinaryFrame`. Cycle 27: `WSType`. Cycle 28: QR `is_private` == Core RFC1918. Cycle 31: shared LAN fixture table (`lan_fixtures.py` / `lan-vectors.json`). |
