...
final class WebSocketServer: ObservableObject {
    ...
    private func accept(_ conn: NWConnection) {
        if connections.count >= ProtocolLimits.maxClients {
            MagicLog.ws("too_many_clients cap=\(ProtocolLimits.maxClients)")
            refuseTooManyClients(conn)
            return
        }
        ...
        ws.onBinary = { [weak self] data in
            ...
            // Cycle 26: seq/tMs come from Core BinaryFrame.parse (same as inject).
            if let frame = BinaryFrame.parse(data), data.count >= BinaryFrame.pointerBytes {
                ws.sendLatencyEcho(seq: frame.seq, tMs: frame.tMs)
            }
            if PairingRuntime.required() && !ws.helloPaired {
                MagicLog.event("binary skipped pairing_required")
            } else {
                InjectRuntime.async {
    ...
    /// 9th (and later) TCP accept: 503 before the WebSocket upgrade.
    /// Does not add the connection; binary 120 Hz of existing clients is untouched.
    private func refuseTooManyClients(_ conn: NWConnection) {
        conn.start(queue: .global(qos: .utility))
        let body = "too_many_clients"
        let resp =
    ...
    private func handleText(_ text: String, from id: ObjectIdentifier) {
        MagicLog.event("text (\(text.count)B)")
    ...
    private func handleJSONText(_ text: String, from id: ObjectIdentifier) {
        guard let data = text.data(using: .utf8),
              ...
              let type = WSType.parse(json["type"] as? String) else { return }

        if JSONRateLimit.meteredTypes.contains(type.rawValue),
           !JSONRateLimit.shared.allow(type.rawValue) {
            sendVoiceAck(to: id, ok: false, reason: "rate_limited")
            MagicLog.event("json rate_limited type=\(type.rawValue)")
            return
        }

        if PairingRuntime.required(),
           type != .hello, type != .ping, type != .classify,
           connections[id]?.helloPaired != true {
            sendVoiceAck(to: id, ok: false, reason: PairingToken.rejectedReason)
            MagicLog.event("json skipped pairing_required type=\(type.rawValue)")
            return
        }
        ...
        case .hello:
            ...
            if !PairingRuntime.allows(json[PairingToken.helloField] as? String) {
                let deny: [String: Any] = [
                    "type": "hello_ack",
                    "ok": false,
                    "reason": PairingToken.rejectedReason,
                    "proto": ProtocolLimits.proto,
                ]
                if let text = JSONText.encode(deny) {
                    connections[id]?.sendText(text)
                }
                MagicLog.event("hello pairing_rejected")
                break
            }
            ...
            let ack: [String: Any] = [
                ...
                "htmlRev": StaticFileLocator.htmlRev(),
                "ax": axNow,
                "clients": n,
                "proto": ProtocolLimits.proto,
            ]
            connections[id]?.helloPaired = true
            if let text = JSONText.encode(ack) {
                connections[id]?.sendText(text)
            }
    ...
    /// Cycle 30: kind via Classify.parseKind; injects stays false.
    private func handleClassify(_ json: [String: Any], from id: ObjectIdentifier) {
        guard !Classify.injects else { return }
        let kind = Classify.parseKind(json["kind"] as? String) ?? ""
    ...
    private func handleSTTControl(_ json: [String: Any], from id: ObjectIdentifier) {
        let action = STTAction.parse(json["action"] as? String)
        let lang = STTLang.parse(json["lang"] as? String)
        let preferOnDevice = STTOnDevice.parse(json["onDevice"])
    ...
    private func handleVoiceText(_ json: [String: Any], from id: ObjectIdentifier) {
        ...
        let ackReason: String
        if !ax {
            ackReason = "ax_denied_clipboard_ok"
        } else if truncated {
            ackReason = "voice_truncated"
        } else {
            ackReason = mode
        }
        InjectTelemetry.note("voice clipboard", ok: true, reason: ackReason)
        MagicLog.event("voice: clipboard chars=\(payload.count) truncated=\(truncated) mode=\(mode) lang=\(lang) ax=\(ax) reason=\(ackReason)")
...
private final class WSConnection: @unchecked Sendable {
    ...
    /// Set under `parseFrame`'s lock; consumed after unlock (docs/CYCLE4-WIRING.md).
    private var pendingCloseCode: UInt16?
    /// Last HTTP Origin for CORS echo. Nil → `*` (curl / missing header).
    private var requestOrigin: String?
    /// 431 sent-once: two receive callbacks can both see an over-cap buffer.
    private var sentHeaderTooLarge = false
    /// Set after a successful hello when a pairing PIN/env is on. Off → unused.
    private var _helloPaired = false
    var helloPaired: Bool {
        get { lock.lock(); defer { lock.unlock() }; return _helloPaired }
        set { lock.lock(); _helloPaired = newValue; lock.unlock() }
    }
    ...
    private func handleData(_ data: Data) {
        ...
        lock.unlock()
        // Pre-handshake `buffer` reads run on the NWConnection receive queue
        // (one callback at a time). parseFrame takes `lock`; this path does not.

        if !handshakeDone {
            // 找 HTTP 头结尾 \r\n\r\n
            let sep = Data([0x0D, 0x0A, 0x0D, 0x0A])
            guard let range = buffer.range(of: sep) else {
                if buffer.count > ProtocolLimits.maxHeaderBytes {
                    if sentHeaderTooLarge { return }
                    sentHeaderTooLarge = true
                    let resp = "HTTP/1.1 431 Request Header Fields Too Large\r\nConnection: close\r\nContent-Length: 0\r\n\r\n"
                    connection.send(content: Data(resp.utf8), completion: .contentProcessed { [weak self] _ in
                        self?.closeInternal()
                    })
                    return
                }
            ...
            if lower.contains("upgrade: websocket") {
                ...
                requestOrigin = HTTPHeaderValue.first(header, name: "origin")
    ...
    private func beginHTTPPost(path: String, header: String) {
        ...
        let lanIPs = LANDetector.allPrivateIPs + [LANDetector.ip]
        if !HTTPPostOrigin.allows(origin, lanIPs: lanIPs) {
            let host = OriginPolicy.host(fromOrigin: origin) ?? "?"
        ...
        // Header only — never `?pair=` (QR / print-only lock).
        let pairHeader = HTTPHeaderValue.first(header, name: "x-magicpad-pair")
        if !PairingRuntime.allows(pairHeader) {
            MagicLog.ws("POST pairing_rejected")
            serveJSON(status: "HTTP/1.1 403 Forbidden", obj: [
                "ok": false, "reason": PairingToken.rejectedReason,
            ])
            return
        }

        let contentLength = HTTPHeaderValue.first(header, name: "content-length").flatMap { Int($0) } ?? 0
        let contentType = HTTPHeaderValue.first(header, name: "content-type") ?? "application/octet-stream"
        let lang = HTTPHeaderValue.first(header, name: "x-magicpad-lang")
            ?? Self.queryValue(path, key: "lang")
            ?? "zh-CN"
        var filename = HTTPHeaderValue.first(header, name: "x-magicpad-filename")
            ?? Self.queryValue(path, key: "name")
            ?? ""
        if let dec = filename.removingPercentEncoding { filename = dec }
        let autoPasteRaw = (HTTPHeaderValue.first(header, name: "x-magicpad-autopaste")
    ...
    /// Methods / cache only. ACAO comes from `corsHeaderBlock` (echo / omit / `*`).
    private static let corsBaseHeaders =
    ...
    private func corsHeaderBlock() -> String {
        let lanIPs = LANDetector.allPrivateIPs + [LANDetector.ip]
        var extra = ""
        if let acao = CORSPolicy.accessControl(origin: requestOrigin, lanIPs: lanIPs) {
            extra += "Access-Control-Allow-Origin: \(acao.allowOrigin)\r\n"
            if acao.needsVary {
                extra += "Vary: Origin\r\n"
            }
        }
        return extra + Self.corsBaseHeaders
    }
    ...
    private func serveStaticFile(path: String, headOnly: Bool = false) {
        ...
        if cleanPath == "/health" || cleanPath == "/healthz" || cleanPath == "/ping" || cleanPath == "/health.json" {
            ...
            if let (data, htmlPath) = StaticFileLocator.loadIndexHTML() {
                ...
                let tried = Array(Set(
                    StaticFileLocator.indexHTMLCandidates().map(StaticFileLocator.sourceLabel)
        ...
        } else if CertRoute.allowsGET(cleanPath) {
            if LANCert.isReady,
               FileManager.default.fileExists(atPath: LANCert.cerURL.path),
               let data = try? Data(contentsOf: LANCert.cerURL),
               !data.isEmpty {
                body = data
                contentType = CertRoute.contentType
                contentDisposition = CertRoute.contentDisposition
            } else {
                statusLine = "HTTP/1.1 404 Not Found"
                body = Data(CertRoute.missingBody.utf8)
                contentType = "text/plain; charset=utf-8"
            }
        } else if CertRoute.refusesSecretExport(cleanPath) {
            statusLine = "HTTP/1.1 404 Not Found"
            body = Data(CertRoute.forbiddenBody.utf8)
            contentType = "text/plain; charset=utf-8"
        } else {
            statusLine = "HTTP/1.1 404 Not Found"
        ...
        if let contentDisposition {
            extra += "Content-Disposition: \(contentDisposition)\r\n"
        }
        if statusLine.contains("503") || statusLine.contains("404") {
            extra += "Content-Security-Policy: \(HTMLEscape.errorPageCSP)\r\n"
            extra += "X-Content-Type-Options: \(HTMLEscape.nosniff)\r\n"
        }
        let responseHead =
            "\(statusLine)\r\n" +
            "Content-Type: \(contentType)\r\n" +
    ...
    /// Cycle 10: JSONText.encode (sorted keys, no \/ ). Keep every smoke-all key.
    /// `ip`/`ips`/`ifaces` stay for LAN debug; CORS echo is the recon control.
    private static func healthJSON() -> String {
        ...
        let obj: [String: Any] = [
            ...
            "https": https,
            "httpsUrl": https ? env.httpsURL : "",
            "httpUrl": env.httpURL,
            "httpsError": LANCert.statusError,
            "ip": LANDetector.ip,
            "ips": LANDetector.allPrivateIPs,
            "ifaces": LANDetector.interfaceSummary,
            "routeIface": env.routeIface,
            "mdns": "",
            "html": resolved != nil,
            "htmlPath": htmlSource,
            "htmlSource": htmlSource,
            "htmlRev": StaticFileLocator.htmlRev(),
            "binaryPath": "MagicPad.app",
            "injectQueue": true,
            ...
            "whisper": true,
            "whisperReady": LocalWhisper.shared.isReady,
            "whisperCached": LocalWhisper.shared.isCached,
            "whisperModel": LocalWhisper.shared.modelLabel,
            "lastKey": InjectTelemetry.last,
            "lastKeyReason": InjectTelemetry.lastReason,
            "injectCount": InjectTelemetry.count,
            "lastKeyOk": InjectTelemetry.lastOk,
            "lastKeyAt": InjectTelemetry.lastAt,
            "lastKeyCount": InjectTelemetry.lastCount,
            "lastDropOk": DropTelemetry.lastOk,
            "lastDropReason": lastDropReason,
            "lastDropKind": DropTelemetry.lastKind,
            "lastDropAt": DropTelemetry.lastAt,
            "dropCount": DropTelemetry.count,
            "lastGesture": GestureTelemetry.lastKind,
            "lastGestureReason": GestureTelemetry.lastReason,
            "lastGestureAt": GestureTelemetry.lastAt,
            "lastGesturePhase": GestureTelemetry.lastPhase,
            "gestureCount": GestureTelemetry.count,
            "clients": WebSocketServer.liveClientCount,
            "ts": Int(Date().timeIntervalSince1970),
            "proto": ProtocolLimits.proto,
        ]
        return JSONText.encode(obj) ?? "{\"ok\":false,\"reason\":\"health_encode\"}"
    }
    ...
    private func handleHandshake(_ header: String) -> HandshakeOutcome {
        let lines = header.components(separatedBy: "\r\n")
        var hasUpgrade = false
        var hasWebSocket = false
        let key = HTTPHeaderValue.first(header, name: "sec-websocket-key")
        let origin = HTTPHeaderValue.first(header, name: "origin")
        for line in lines {
        ...
        let allowAny = ProcessInfo.processInfo.environment["MAGICPAD_ALLOW_ANY_ORIGIN"] == "1"
        let lanIPs = LANDetector.allPrivateIPs + [LANDetector.ip]
        if !allowAny && !OriginPolicy.isAllowed(origin: origin, lanIPs: lanIPs) {
            let host = OriginPolicy.host(fromOrigin: origin) ?? "?"
            MagicLog.ws("handshake origin rejected host=\(host)")
            let forbidden = "HTTP/1.1 403 Forbidden\r\nConnection: close\r\nContent-Length: 0\r\n\r\n"
            connection.send(content: forbidden.data(using: .utf8), completion: .contentProcessed { [weak self] _ in
                self?.closeInternal()
            })
            return .closing
        }
        let version = HTTPHeaderValue.first(header, name: "sec-websocket-version")
        if version != ProtocolLimits.requiredWebSocketVersion {
            MagicLog.ws("handshake version rejected")
            let resp = "HTTP/1.1 426 Upgrade Required\r\nSec-WebSocket-Version: \(ProtocolLimits.requiredWebSocketVersion)\r\nConnection: close\r\nContent-Length: 0\r\n\r\n"
            connection.send(content: Data(resp.utf8), completion: .contentProcessed { [weak self] _ in
                self?.closeInternal()
            })
            return .closing
        }
        guard hasUpgrade, hasWebSocket, let key, !key.isEmpty else {
    ...
    private func parseFrame() -> Frame? {
        ...
        case 127:
            ...
            guard v <= UInt64(Int.max) else {
                // RFC 6455 §5.2: most-significant bit of the 64-bit length must be 0.
        ...
        // lock held — do not close here (docs/CYCLE4-WIRING.md)
    ...
    private func sendCloseFrame(code: UInt16) {
        var frame = Data()
        frame.append(0x88) // FIN + close
