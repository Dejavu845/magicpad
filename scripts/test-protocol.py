...
class Cycle18HelloPingTests(unittest.TestCase):
    ...
    def test_protocol_hello_ping_fields(self):
        proto = Path(os.path.dirname(HERE), "docs", "PROTOCOL.md").read_text(
            encoding="utf-8"
        )
        self.assertIn("### hello / ping", proto)
        for needle in ("hello_ack", "htmlRev", "serverTs", "No pairing token"):
...
class Cycle11LogRedactTests(unittest.TestCase):
    ...
    def test_logger_chmod_600(self):
        path = os.path.join(
            os.path.dirname(HERE),
            "MagicPadServer",
            "Sources",
        ...
        raw = Path(path).read_text(encoding="utf-8")
        self.assertIn("0o600", raw)

    def test_no_payload_prefix_in_websocket_logs(self):
        path = os.path.join(
            os.path.dirname(HERE),
            "MagicPadServer",
            "Sources",
        ...
        if len(raw.encode("utf-8")) < 20_000:
            self.skipTest("WebSocketServer.swift is the remote stub")
        self.assertNotIn("text.prefix(200)", raw)
